...
The helpers prefer Gio when it has the data, then fall back to explicit file
parsing so features keep working across classic installs, host-mounted
sandboxes, Snap desktop exports, and user-generated launchers.
"""
...
def desktop_match_aliases(info: DesktopInfo) -> list[str]:
    """Compatibility delegate accepting the historical ``DesktopInfo`` tuple."""
    return match_aliases(info.desktop_id, info.wm_class, info.exec_line)
