...
def _attach_runtime_methods(harness: _Harness) -> None:
    ...
    harness._get_geometry_frame = MethodType(
        dock_window_mod.DockWindow._get_geometry_frame, harness
    )
    harness._current_input_rect = MethodType(
        dock_window_mod.DockWindow._current_input_rect, harness
    )
...
def _hover_first_item(harness: _Harness, widget: MagicMock) -> tuple[float, float]:
    frame = harness._get_geometry_frame(main_cursor=330.0)
...
def _hover_last_item(harness: _Harness, widget: MagicMock) -> tuple[float, float]:
    frame = harness._get_geometry_frame(main_cursor=1590.0)
