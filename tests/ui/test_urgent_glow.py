...
class TestUrgentGlowOpacity:
    ...
    def test_zero_elapsed_is_midpoint(self):
        ...
        opacity = compute_urgent_glow_opacity(
            elapsed_us=0, glow_time_ms=10000, pulse_ms=2000
        )
    ...
    def test_quarter_pulse_is_peak(self):
        ...
        opacity = compute_urgent_glow_opacity(
            elapsed_us=quarter_us, glow_time_ms=10000, pulse_ms=2000
        )
    ...
    def test_three_quarter_pulse_is_trough(self):
        ...
        opacity = compute_urgent_glow_opacity(
            elapsed_us=three_quarter_us, glow_time_ms=10000, pulse_ms=2000
        )
    ...
    def test_expired_returns_zero(self):
        ...
        opacity = compute_urgent_glow_opacity(
            elapsed_us=11_000_000, glow_time_ms=10000, pulse_ms=2000
        )
        assert opacity == 0.0

    def test_negative_elapsed_returns_zero(self):
        assert (
            compute_urgent_glow_opacity(
                elapsed_us=-1, glow_time_ms=10000, pulse_ms=2000
            )
            == 0.0
        )
    ...
    def test_at_exact_glow_time_returns_zero(self):
        ...
        opacity = compute_urgent_glow_opacity(
            elapsed_us=10_000_000, glow_time_ms=10000, pulse_ms=2000
        )
        assert opacity == 0.0
    ...
    def test_oscillates_over_time(self):
        ...
        values = [
            compute_urgent_glow_opacity(
                elapsed_us=t * 100_000, glow_time_ms=10000, pulse_ms=2000
            )
            for t in range(20)
        ]
